# Exemplo de Consultas PostgreSQL
...
Incluidados da empresa, endereço, quadro societário e informações do Simples Nacional.
...
SELECT
    ...
    (CASE WHEN est.data_inicio_atividade > '0001-01-01' THEN to_char(est.data_inicio_atividade, 'DD/MM/YYYY') ELSE '' END) AS "Data de Abertura",
    ...
    (CASE WHEN est.data_situacao_cadastral > '0001-01-01' THEN to_char(est.data_situacao_cadastral, 'DD/MM/YYYY') ELSE '' END) AS "Data da Situação Cadastral",
    ...
    (COALESCE((
        SELECT STRING_AGG(sc.cod_cnae || ' - ' || COALESCE(cn_sec.nome_cnae, 'CNAE SECUNDÁRIO DESCONHECIDO'), ' / ')
    ...
    (CASE WHEN sn.data_opcao_simples > '0001-01-01' THEN to_char(sn.data_opcao_simples, 'DD/MM/YYYY') ELSE '' END) AS "Data Opção Simples",
    (CASE WHEN sn.data_exclusao_simples > '0001-01-01' THEN to_char(sn.data_exclusao_simples, 'DD/MM/YYYY') ELSE '' END) AS "Data Exclusão Simples",
    (CASE COALESCE(sn.opcao_mei, '') WHEN 'S' THEN 'SIM' ELSE 'NÃO' END) AS "Optante MEI",
    (CASE WHEN sn.data_opcao_mei > '0001-01-01' THEN to_char(sn.data_opcao_mei, 'DD/MM/YYYY') ELSE '' END) AS "Data Opção MEI",
    (CASE WHEN sn.data_exclusao_mei > '0001-01-01' THEN to_char(sn.data_exclusao_mei, 'DD/MM/YYYY') ELSE '' END) AS "Data Exclusão MEI",
    (TRIM(
    ...
    (est.email) AS "E-mail",
    ((
        SELECT STRING_AGG(UPPER(TRIM(s.nome_socio)), ', ')
        FROM socio s
